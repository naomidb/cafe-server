...
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    ...
    try:
        r = requests.request('POST', settings.BASE_URL, data=body, headers=headers)
        if r.ok:
            try:
                data = r.json()
                terms = []
...
def run_statements(statements, context):
    ...
    try:
        r = requests.request('PUT', settings.BASE_URL + '/statements', data=body, headers=headers, params=params)
